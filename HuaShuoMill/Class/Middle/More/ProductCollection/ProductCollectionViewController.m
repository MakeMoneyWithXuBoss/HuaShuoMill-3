...
#define  ProductCollectionCellID @"ProductCollectionCell"
...
@interface ProductCollectionViewController ()<UICollectionViewDataSource,UICollectionViewDelegate>
{
    int pageindex;
}
...
- (void)setupConfigeCollectionView
{
    ...
    UINib *nib = [UINib nibWithNibName:ProductCollectionCellID bundle:nil];
    [self.collectionView registerNib:nib forCellWithReuseIdentifier:ProductCollectionCellID];
}
...
- (void)loadData:(NSNumber*)more
{
    ...
    NSString *urlString = [NSString stringWithFormat:@"%@%@",API_products_getAll_category_id,@"1"];
    ...
    [XHMDataService getRequestWithURL:urlString params:params isShowLoading:YES successBlock:^(id result) {
        ...
        for (NSMutableDictionary *categoriesDic in categoriesArray) {
            ProductCollectionModel *productBrandModel = [[ProductCollectionModel alloc] initWithDic:categoriesDic];
...
{
    //1.获得cell
    ProductCollectionCell *cell = (ProductCollectionCell *)[collectionView dequeueReusableCellWithReuseIdentifier:ProductCollectionCellID forIndexPath:indexPath];
    
    
    ProductCollectionModel *productBrandModel = [self.dataArray objectAtIndex:indexPath.row];
